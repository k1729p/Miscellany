...
public class ApplicationForStreamsCollecting {
	...
	private static void collectStream() {
		...
		final Supplier<Stream<String>> supplier = () -> List.of("DBCADBCA".split("")).stream().sorted();
		final CharSequence delimiter = "█";
		final CharSequence prefix = "►";
		final CharSequence suffix = "◄";

		// equivalent to: reducing(0L, e -> 1L, Long::sum)
		Printer.printf("Counting[%s]", supplier.get()//
				.collect(Collectors.counting()));
		Printer.printf("Reducing[%s]", supplier.get()//
				.reduce(String::concat).orElse("ERROR"));
		Printer.printf("Joining [%s]%n", supplier.get()//
				.collect(Collectors.joining(delimiter, prefix, suffix)));

		Printer.printf("List    %s (unmodifiable list)", supplier.get()//
				.toList());
		Printer.printf("List    %s (modifiable list)", supplier.get()//
				.collect(Collectors.toCollection(ArrayList::new)));
		Printer.printf("List    %s (from distinct stream)", supplier.get()//
				.distinct().toList());
		Printer.printf("Set     %s", supplier.get()//
	...
	private static void countWithoutPipelineExecution() {
		...
		final Supplier<Stream<String>> supplier1 = () -> List.of("12345678".split("")).stream().map(operator);
