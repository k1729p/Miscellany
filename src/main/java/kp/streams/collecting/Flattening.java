...
public class Flattening {
	 ...
	 * <p>
	 * The method {@link Stream#mapMulti} is <b>preferable</b> to the method
	 * {@link Stream#flatMap} in some circumstances.
	 */
	static void compareMapMultiVersusFlatMapForNumbers() {
		...
		final Function<Integer, Stream<Double>> mapperForFlat = num -> Stream.of(0.8 * num, 1.0 * num, 1.2 * num);
		...
		final BiConsumer<Integer, Consumer<Double>> mapperForMulti = (num, cons) -> mapperForFlat.apply(num)
				.forEach(cons);
	...
	static void compareMapMultiVersusFlatMapForStrings() {
		...
		final BiConsumer<String, Consumer<String>> mapperForMulti = (num, cons) -> mapperForFlat.apply(num)
				.forEach(cons);
		Printer.print("Using 'mapMulti()':");
