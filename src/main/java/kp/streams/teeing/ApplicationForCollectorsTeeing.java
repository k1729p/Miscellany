...
public class ApplicationForCollectorsTeeing {
	...
	private static void processZonedDateTime() {
		...
		final BinaryOperator<Map<String, Map<String, List<String>>>> merger = (map1, map2) -> {
			...
			Stream.of(map1, map2).forEach(srcMap -> srcMap.forEach((key, value) -> {
				final Map<String, List<String>> innerMergedMap = mergedMap.getOrDefault(key, new TreeMap<>());
				mergedMap.putIfAbsent(key, innerMergedMap);
				value.forEach((key1, value1) -> {
					final List<String> list = innerMergedMap.getOrDefault(key1, new ArrayList<>());
					innerMergedMap.putIfAbsent(key1, list);
					list.addAll(value1);
				});
			}));
			return mergedMap;
		...
		resultMap.forEach((key, value) -> {
			...
			value.forEach((key1, value1) -> {
				Printer.printf("\t" + VAL_FMT, key1);
				value1.forEach(text -> Printer.printf("\t\t" + VAL_FMT, text));
			});
		});
		Printer.printHor();
	}
	...
	private static void processAtomicSimple() {
		...
		final BinaryOperator<Map<String, List<String>>> merger = (map1, map2) -> {
			...
			Stream.of(map1, map2).forEach(//
					srcMap -> srcMap.forEach((key, value1) -> {
						final List<String> list = mergedMap.getOrDefault(key, new ArrayList<>());
						mergedMap.putIfAbsent(key, list);
						value1.forEach(//
	...
	private static void processAtomicDetailed() {
		...
		final Function<String, BinaryOperator<Map<String, List<String>>>> mergerFun = label -> (map1, map2) -> {
			...
			final TreeMap<String, List<String>> mergedMap = new TreeMap<>();
			Stream.of(map1, map2).forEach(srcMap -> srcMap.forEach((key, value1) -> {
				final List<String> list = mergedMap.getOrDefault(key, new ArrayList<>());
				mergedMap.putIfAbsent(key, list);
