...
public class ApplicationForCollectorsTeeing {
	...
	private static final Function<ZonedDateTime, String> DATE_KEY_FUN = zdt -> String.format(DATE_TIME_FMT, zdt);
	...
	private static void processJoining() {
		...
		final BiFunction<String, List<String>, List<String>> merger = (joinedString, stringList) -> {
			stringList.add(joinedString);
			return stringList;
		};
		final Collector<String, ?, List<String>> compositeCollector = Collectors.teeing(//
				Collectors.joining("-"), Collectors.toList(), merger);
	...
	private static void processZonedDateTime() {
		...
		final BiConsumer<Map<String, Map<String, List<String>>>, ZonedDateTime> accumulator1 = (map, zonedDateTime) -> {
			final String dateKey = DATE_KEY_FUN.apply(zonedDateTime);
			final Map<String, List<String>> dateMap = map.getOrDefault(dateKey, new TreeMap<>());
			map.putIfAbsent(dateKey, dateMap);
			final String listKey = String.format("'%s' in month '%s'",
			...
			final List<String> list = dateMap.getOrDefault(listKey, new ArrayList<>());
			dateMap.putIfAbsent(listKey, list);
		...
		final BiConsumer<Map<String, Map<String, List<String>>>, ZonedDateTime> accumulator2 = (map, zonedDateTime) -> {
			final String dateKey = DATE_KEY_FUN.apply(zonedDateTime);
			final Map<String, List<String>> dateMap = map.getOrDefault(dateKey, new TreeMap<>());
			map.putIfAbsent(dateKey, dateMap);
			final String listKey = "formatted date & time";
			final List<String> list = dateMap.getOrDefault(listKey, new ArrayList<>());
			dateMap.putIfAbsent(listKey, list);
			...
			dateTimeFormatter = DateTimeFormatter.ofPattern("qqqq 'of year', w 'week of year', eeee B");
			list.add(zonedDateTime.format(dateTimeFormatter));
		};
		final BiConsumer<Map<String, Map<String, List<String>>>, ZonedDateTime> accumulator3 = (map, zonedDateTime) -> {
			final String dateKey = DATE_KEY_FUN.apply(zonedDateTime);
			final Map<String, List<String>> dateMap = map.getOrDefault(dateKey, new TreeMap<>());
			map.putIfAbsent(dateKey, dateMap);
			final String listKey = "six days earlier & six days later";
			final List<String> list = dateMap.getOrDefault(listKey, new ArrayList<>());
			dateMap.putIfAbsent(listKey, list);
			list.add(String.format(DATE_FMT, zonedDateTime.minus(6, ChronoUnit.DAYS)));
			list.add(String.format(DATE_FMT, zonedDateTime.plus(6, ChronoUnit.DAYS)));
		};
		final BiConsumer<Map<String, Map<String, List<String>>>, ZonedDateTime> accumulator4 = (map, zonedDateTime) -> {
			final String dateKey = DATE_KEY_FUN.apply(zonedDateTime);
			final Map<String, List<String>> dateMap = map.getOrDefault(dateKey, new TreeMap<>());
			map.putIfAbsent(dateKey, dateMap);
			final String listKey = "six hours earlier & six hours later";
			final List<String> list = dateMap.getOrDefault(listKey, new ArrayList<>());
			dateMap.putIfAbsent(listKey, list);
			list.add(String.format(DATE_TIME_FMT, zonedDateTime.minus(6, ChronoUnit.HOURS)));
			list.add(String.format(DATE_TIME_FMT, zonedDateTime.plus(6, ChronoUnit.HOURS)));
		};
		final BinaryOperator<Map<String, Map<String, List<String>>>> merger = (map1, map2) -> {
			final TreeMap<String, Map<String, List<String>>> mergedMap = new TreeMap<>();
			Stream.of(map1, map2).forEach(srcMap -> srcMap.entrySet().forEach(entrySet -> {
				final Map<String, List<String>> innerMergedMap = mergedMap.getOrDefault(entrySet.getKey(),
						new TreeMap<>());
				mergedMap.putIfAbsent(entrySet.getKey(), innerMergedMap);
				entrySet.getValue().entrySet().forEach(innerEntrySet -> {
					final List<String> list = innerMergedMap.getOrDefault(innerEntrySet.getKey(), new ArrayList<>());
					innerMergedMap.putIfAbsent(innerEntrySet.getKey(), list);
					list.addAll(innerEntrySet.getValue());
				});
			}));
			return mergedMap;
		};
		...
		resultMap.entrySet().forEach(entrySet -> {
			Printer.printf(VAL_FMT, entrySet.getKey());
			entrySet.getValue().entrySet().forEach(innerEntrySet -> {
				Printer.printf("\t" + VAL_FMT, innerEntrySet.getKey());
				innerEntrySet.getValue().forEach(text -> Printer.printf("\t\t" + VAL_FMT, text));
			});
		});
		Printer.printHor();
	}
	...
	private static void processAtomicSimple() {
		...
		final BinaryOperator<Map<String, List<String>>> merger = (map1, map2) -> {
			...
			Stream.of(map1, map2).forEach(//
					srcMap -> srcMap.entrySet().forEach(entrySet -> {
						final List<String> list = mergedMap.getOrDefault(entrySet.getKey(), new ArrayList<>());
						mergedMap.putIfAbsent(entrySet.getKey(), list);
						entrySet.getValue().forEach(//
								value -> list.add(String.format("%s %02d-mrg ", value, atomicMrg.incrementAndGet())));
					}));
			return mergedMap;
		};
		...
		resultMap.entrySet().forEach(entrySet -> {
			Printer.printf(VAL_FMT, entrySet.getKey());
			entrySet.getValue().forEach(value -> Printer.printf("\t" + VAL_FMT, value));
		});
		Printer.printHor();
	}
	...
	private static void processAtomicDetailed() {
		...
		final Function<String, BiConsumer<Map<String, List<String>>, String>> accumulatorFun = label -> (map, key) -> {
			final List<String> list = map.getOrDefault(key, new ArrayList<>());
			map.putIfAbsent(key, list);
			list.add(String.format("%s-%02d ", label, atomicAcc.incrementAndGet()));
		};

		final Function<String, BinaryOperator<Map<String, List<String>>>> mergerFun = label -> (map1, map2) -> {
			...
			Stream.of(map1, map2).forEach(srcMap -> srcMap.entrySet().forEach(entrySet -> {
				...
				entrySet.getValue().forEach(value -> {
					final String result = String.format("%s %s-%02d-%02d ", value, label, atomicRes.incrementAndGet(),
							mergeNumber);
					list.add(result);
				});
			}));
