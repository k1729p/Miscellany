...
public class TemporaryFilesAndZipFiles {
	...
	private static final String TMP_DIR_VALUE = "c:\\Temp";
	private static final List<String> CONTENT_LIST = List.of("The first line.", "The second line.");
	...
	public TemporaryFilesAndZipFiles() {
		super();
		final Path previousTmpDir = Paths.get(System.getProperty(TMP_DIR_KEY));
		System.setProperty(TMP_DIR_KEY, TMP_DIR_VALUE);
		Printer.printf("Temporary directory from property 'java.io.tmpdir': previous[%s], actual[%s]", previousTmpDir,
				Paths.get(System.getProperty(TMP_DIR_KEY)));
		Printer.printHor();
	}
	...
	public void writeAndReadTemporaryFiles() {
		...
		try {
			final Path tempDirectory = Files.createTempDirectory(Path.of(System.getProperty(TMP_DIR_KEY)), "example");
			...
			Files.write(fileInTempDir, CONTENT_LIST, Charset.defaultCharset());
			...
			try (Stream<Path> pathStream = Files.walk(tempDirectory)) {
				pathStream.sorted(Comparator.reverseOrder()).map(Path::toFile).map(printMapper).forEach(File::delete);
			}

			final String[] prefixAndSuffix = ConstantsForFiles.EXAMPLE_TXT_FILE_NAME.split("\\.");
			final Path tempFile = Files.createTempFile(Path.of(System.getProperty(TMP_DIR_KEY)), prefixAndSuffix[0],
					".".concat(prefixAndSuffix[1]));
			...
			try (BufferedWriter bufferedWriter = Files.newBufferedWriter(tempFile)) {
				for (String line : CONTENT_LIST) {
					bufferedWriter.write(line.concat(LINE_SEP));
				}
			}
	...
	public void writeAndReadTemporaryZipFiles() {
		...
		try {
			...
			final Path tempFile = Files.createTempFile(Path.of(System.getProperty(TMP_DIR_KEY)), prefix, suffix);
