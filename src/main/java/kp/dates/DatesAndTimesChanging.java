...
public class DatesAndTimesChanging {
	...
	public void convertDateToAndFro() {
		...
		java.time.ZonedDateTime zonedDateTime1 = ZonedDateTime.ofInstant(EXAMPLE_DATE.toInstant(),
				ZoneId.systemDefault());
		Printer.printf("localDate\t[%s]%nlocalTime\t\t   [%s]%nlocalDateTime\t[%s]%nzonedDateTime\t[%s]", localDate1,
				localTime1, localDateTime1, zonedDateTime1);

		// <<<
		@SuppressWarnings("unused")
		java.util.Date utilDate2 = Date.from(localDate1.atStartOfDay().atZone(ZoneId.systemDefault()).toInstant());
		@SuppressWarnings("unused")
		java.util.Date utilDate3 = Date.from(localDate1.atTime(localTime1).atZone(ZoneId.systemDefault()).toInstant());
		@SuppressWarnings("unused")
		java.util.Date utilDate4 = Date.from(localDateTime1.atZone(ZoneId.systemDefault()).toInstant());
		@SuppressWarnings("unused")
		java.util.Date utilDate5 = Date.from(zonedDateTime1.toInstant());
		...
		Printer.printf("sqlDate\t\t[%s]%nsqlTimestamp\t[%s]", sqlDate, sqlTimestamp);

		// <<< SQL
		@SuppressWarnings("unused")
		java.util.Date utilDate6 = java.sql.Date.valueOf(localDate1);
		@SuppressWarnings("unused")
		java.util.Date utilDate7 = java.sql.Timestamp.valueOf(localDateTime1);
	...
	public void adjustDate() {
		...
		adjusterMap.entrySet().forEach(entry -> {
			final LocalDate localDate = Constants.EXAMPLE_LOCAL_DATE.with(entry.getValue());
			Printer.printf("Adjusted as %-23s: day of week[%9s], day of month[%2d], month[%9s]", entry.getKey(),
	...
	public void queryTemporalObjects() {
		...
		localDateTimeList.stream().forEach(temporal -> Printer.printf(//
	...
	public void calculateAmountOfTimeBetween() {
		...
		Printer.printf("Chrono units between start time[%1$tF %1$tT] and end time[%2$tF %2$tT]:%n"//
				+ "\thours[%3$d] or minutes[%4$d] or seconds[%5$s] or millis[%6$s]", startDateTime, endDateTime1,
		...
		Printer.printf("Chrono units between start time[%1$tF %1$tT] and end time[%2$tF %2$tT]:%n"//
				+ "\tmonths[%3$d] or weeks[%4$d] or days[%5$d]", startDateTime, endDateTime2,
		...
		Printer.printf("Duration between start time[%s] and end time[%s]:%n"//
				+ "\tduration[%s], seconds[%s] and nanos[%s]", startTime, endTime, duration1, duration1.getSeconds(),
				duration1.getNano());
