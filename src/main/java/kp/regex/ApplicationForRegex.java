...
public class ApplicationForRegex {
	...
	private static void showGreedyReluctantPossessiveQuantifiers() {
		...
		dataMap.entrySet().stream().forEach(consumer);
		Printer.printHor();
	}
	...
	private static void showNamedCapturingGroups() {
		...
		if (matcher.find()) {
			Printer.printf("\tyear[%s], month[%s], day[%s], date[%s], day of the week[%s]", matcher.group("Year"),
					matcher.group("Month"), matcher.group("Day"), matcher.group("Date"), matcher.group("DayOfWeek"));
			Printer.printf("\thour[%s], minute[%s], second[%s], time[%s], time zone[%s]", matcher.group("Hour"),
					matcher.group("Minute"), matcher.group("Second"), matcher.group("Time"), matcher.group("TimeZone"));
		}
		Printer.printHor();
	}
